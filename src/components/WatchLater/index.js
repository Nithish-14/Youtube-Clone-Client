...
const WatchLater = () => {
    const [watchLaterVideos, addWatchLaterVideos] = useState([])
    ...
    const renderSignIn = () => (
        <LoaderContainer>
            <FailureViewPara>Watchlater isn't viewable when signed out</FailureViewPara>
    ...
    const renderEmptyHistory = () => (
        <LoaderContainer>
            <FailureViewHeading>No videos have been watched later.</FailureViewHeading>
        </LoaderContainer>
    )
    ...
    const renderVideos = async () => {
        ...
        const response = await fetch("https://youtube-clone-server-lqs9.vercel.app/user/watchlater", options);
        if (response.ok) {
    ...
    const renderSuccessView = () => {
        if (watchLaterVideos.length === 0) {
            ...
            return watchLaterVideos.map((each, idx) => <VideoCard key={idx} videoItem={each} />)
        }
    }
                        ...
                        <HomeContainer>
                            ...
                            <HorizontalScrollContainer width={showSidebar.toString()}>
                                <RouteHeadingContainer>
                                    <RouteHeading>Watch later</RouteHeading>
                                </RouteHeadingContainer>
